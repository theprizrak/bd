...
<input type="text" name="date_dobav" class="add" value="<?=date('d.m.Y');?>" style="width: 1px;height: 1px;display: none;">
  ...
  <tr>
    ...
    <td width="67%">
    <input type="text" class="add" value="<?=$vozrast;?>" name="vozrast" readonly>
    <div class="edit_vstr ok_vorz" style="display:none"><img title="Сохранить" src="<?= base_url();?>/images/ok.png" width="20" height="20" onclick="edit_vozrast('ok')" style="cursor: pointer;"/></div><div class="edit_vstr edit_vozr" style="display: inline-block;"><img title="Редактировать возраст" src="<?= base_url();?>/images/edit_vstr.png" width="20" height="20" onclick="edit_vozrast('edit')" style="cursor: pointer; "/></div>
   </td>
  </tr>
  <tr>
  ...
  <td colspan="2">
    ...
    if($klientss_vstr!=0)
    {
        ...
        <td><?if($klientus_vstr['otkaz']==0){?><div class="edit_vstr"><img title="Отказ от встречи" src="<?= base_url();?>/images/otkz.png" width="20" height="20" onclick="otkz_vstr_tm('<?=$klientus_vstr['id'];?>','<?=$id;?>')" style="cursor: pointer;"/></div><?}?></td>        
        <td><?if($klientus_vstr['otkaz']==0){?><div class="edit_vstr"><img title="Удалить встречу" src="<?= base_url();?>/images/otkz.png" width="20" height="20" onclick="del_vstr_tm('<?=$klientus_vstr['id'];?>','<?=$id;?>')" style="cursor: pointer;"/></div><?}?></td>
        <?}else{?>
        <td></td>
        <td></td>
        <td></td>
  ...
  <td colspan="6">Комментарий: <input type="text" name="comments" style="border: 1px solid #ccc;  border-radius: 3px;  padding: 2px;  width: 560px;" value=""></td>
    ...
    <tr><td colspan="14" align="center" class="ss1" style="height: 40px;"><a class="buttons" id="avst" onclick="newvstr_tm()">Добавить встречу</a></td></tr>
    </tbody>
    </table>
  </td>
  </tr>
  ...
  <tr class="Sel3" style="display: none;background: #A8F3AE;text-align: center;">
    <td height="23" colspan="2" style="padding: 10px;"><a class="buttons" id="svst" onclick="savevstr()">Сохранить встречу</a><a class="buttons" id="svst" onclick="cancel()">Отмена</a></td>
  </tr>
  ...
  <td colspan="2">
    ...
    <tr><td colspan="4" align="center" class="ss2" style="height: 40px;"><a class="buttons" id="azvon" onclick="newzvon()">Добавить Звонок</a></td></tr>
		<tr class="Selp" style="display: none;background: #EDF3A8;text-align: center;">
			<td height="23" colspan="4" style="padding: 10px;"><a class="buttons" id="svst" onclick="newpzvon()">Сохранить перезвон</a><a class="buttons" id="svst" onclick="cancel()">Отмена</a></td>
		</tr> 
   </tbody>
    </table>
