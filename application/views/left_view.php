...
<? if($this->session->userdata('doljn')=='IT' || $this->session->userdata('doljn')=="Рук-ль тм1"){?>
...
<span class="user">
<?
$users_name=preg_replace_callback( '/^(\S)(.*)$/u', function( $a ) { return utf8_encode(chr($a)).strtoupper($a[1]).strtolower($a[2]); }, $this->session->userdata('family'))." ".preg_replace_callback( '/^(\S)(.*)$/u', function( $b ) { return utf8_encode(chr($b)).strtoupper($b[1]).strtolower($b[2]);  }, $this->session->userdata('name'));

/*$users_name=preg_replace_callback( '/^(\S)(.*)$/eu', function( $a ) { return utf8_encode(chr( $a ) ); }{"mb_strtoupper('\\1', 'UTF-8').mb_strtolower('\\2', 'UTF-8')"}, $this->session->userdata('family'))." ".preg_replace( '/^(\S)(.*)$/eu', "mb_strtoupper('\\1', 'UTF-8').mb_strtolower('\\2', 'UTF-8')", $this->session->userdata('name'));*/
echo $users_name;?>
</span>
