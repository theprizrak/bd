...
function phones(){
    if($("div").is("#phoneInfo")){}else{$('body').append('<div id="phoneInfo" class="info"></div>');}
...
function addClient(){
    ...
    else
    {
        ...
        {
			if(vstr==0 && perz==0){alert("Укажите встречу или перезвон");exit();}
            if(vstr==1)
            {
    ...
    if(date=='ok')
        {
            ...
            type: "POST",
            url: "edit_vozrast",
            data: {"id": id, "vozrast": vozrast, "safe_ok": 'safe_ok'},
            cache: false,
            success: function(status){
            var messageResp = new Array('Возраст изменен','Ошибка. Обратитесь к администратору');
            var resultStat = messageResp[Number(status)];
            if(status == 0){
            $('#info').text(resultStat).show('blind').delay(1500).fadeOut(800);
            $('#preloader').css('display','none'); 
            //$('#blackFon').css('display','none');
            setTimeout("window.location = '/telemarketing/edit?id="+id+"'", 2000)
            }
            else if(status == 1){
            $('#info').text(resultStat).show('blind').delay(1500).fadeOut(800);
            $('#preloader').css('display','none');
        ...
        {
            $(".ok_vorz").css("display","inline-block");
            $("input[name=vozrast]").removeAttr("readonly").attr("style","border: 1px solid rgb(245, 0, 0);");
        }
