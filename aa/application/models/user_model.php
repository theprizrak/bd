...
return $date;    
    ...
    function add_przv($data)
  {
    ...
    //выбираем назначенный план на сегодня сотруднику
     ...
     //выбираем колличество встреч назначеных на сегодня
  ...
  function save_klient_sresep($data)
  {
    ...
    return $query->result_array();
     ...
     //получаем число назначенных пользователем встреч
      ...
      //получаем число назначенных пользователем встреч за сегодня
     function get_nazn_today($date) {
        $date=$this->rotate_date($date);
      ...
      //получаем число отказанных пользователем встреч за сегодня
     function get_otkz_today($date) {
        $date=$this->rotate_date($date);
        if($date==0){$date=date("Y-m-d");}
    ...
    function get_user_list($id_user_out)
    {
        $query=$this->db->get('session');
        ...
        {
            $this->db->where('id_in',$item['id_user']);
            $this->db->where('id_out',$id_user_out);
            $this->db->where('mail_ok',1);
            $query_mail = $this->db->get('mail');
        }
        $query_ok=array();
        $query_ok['query']=$query;
        $query_ok['query_mail']=$query_mail;
        return $query_ok;  */
        return $query->result_array();
    }
            //получаем список непрочтенных сообщений
    function get_count_mail_ok($id_user_out,$id_user_in)
    {
        $this->db->where('id_in',$id_user_out);
        $this->db->where('id_out',$id_user_in);
        $this->db->where('mail_ok',1);
        $query_mail = $this->db->get('mail');
        return $query_mail->result_array();  
        //return $query->result_array();
    }
                //получаем список непрочтенных сообщений
    function get_all_count_mail_ok($id_user_out)
    {
        $this->db->where('id_in',$id_user_out);
        $this->db->where('mail_ok',1);
        $query_mail = $this->db->get('mail');
        //if(count($query_mail)!=0){return true;}else{return false;}
        return $query_mail->result_array();  
        //return $query->result_array();
    }
    
    //ставим статус что сообщение прочитано
    function mail_ok($id_user_in,$id_user_out)
    {
        $this->db->where('id_in',$id_user_out);
        $this->db->where('id_out',$id_user_in);
        $this->db->set('mail_ok',0);
        $query = $this->db->update('mail');
        if($query){return true;}else{return false;} 
    }
     
   //сохраняем информационное сообщение 
    function safe_info_msg($text_msg,$color_bg_msg)
    {
        $this->db->set('text_msg',$text_msg);
        $this->db->set('color_bg_msg',$color_bg_msg);
        $query = $this->db->update('config');
        if($query){return true;}else{return false;}
    }
