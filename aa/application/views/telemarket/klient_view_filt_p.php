...
<strong>Клиент/Контакт не найден!</strong>
    ...
    foreach ($ag as $vsstr):
        $id=$vsstr;
        $userrrrs='';
        $klientus=$this->telemarket_model->get_klients_tel3($userrrrs,$id);
    if(!empty($klientus))
    {
	...
	<td><?if(!empty($klientss_vstr)){if(isset($klientss_vstr['date_vstr']) || isset($klientss_vstr['time_vstr'])){if($klientss_vstr['date_vstr']!='' || $klientss_vstr['time_vstr']!=''){echo $klientss_vstr['date_vstr'].' '.$klientss_vstr['time_vstr'];}}}?></td>
