...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tableList sortTable">
  ...
  <tbody>
   ...
   if (isset($_COOKIE["info"]) && $_COOKIE["info"]==2){
    ...
    $klientss_vstr=$this->telemarket_model->get_klients_vstr_tm($klientus['id']);
    if($klientus['otkaz_tm']==1 || isset ($klientss_vstr['otkaz']) && $klientss_vstr['otkaz']==1)
    {$status='id="fall"';}
    elseif($klientus['perezvon_tm']==1)
    {$status='id="call"';}
  ...
  <tr <?if(isset($status)){echo $status;}?> data-href="/telemarketing/edit?id=<?=$klientus['id'];?>">
    ...
    <td><?if($klientus['perezvon_tm']!=1){if(!empty($klientss_vstr)){if($klientss_vstr['prodacha']==1){echo'Да';}else{echo'Нет';}} }?></td>
	<td><?if($klientus['perezvon_tm']!=1){if(!empty($klientss_vstr)){if(isset($klientss_vstr['date_vstr']) || isset($klientss_vstr['time_vstr'])){if($klientss_vstr['date_vstr']!='' || $klientss_vstr['time_vstr']!=''){echo $this->user_model->rotate_date2($klientss_vstr['date_vstr']).' '.$klientss_vstr['time_vstr'];}}} }?></td>
    <td><?if($klientus['perezvon_tm']!=1){if(!empty($klientss_vstr)){if($klientss_vstr['predpod']==1){echo'Да';}else{echo'Нет';}} }?></td>
	<td><?if($klientus['perezvon_tm']!=1){if(isset($klientss_vstr['comments'])){if($klientss_vstr['comments']!=''){echo $klientss_vstr['comments'];}} }?></td>
  </tr>
    <?php endforeach; }?>
