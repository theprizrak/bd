...
function asds()
{
    var s=$('input[name=proc]').length;
    //var asd=$('input[name=proc[]]:checked').val;
    var ads="";
    
    $aDoor =$('input[name=proc[]+]');
    $N=$aDoor.length;
    alert($aDoor.length);
    for($i=0; $i < $N; $i++)
    {
   ...
   /* var i = 1;
    while(i<s) {
        ...
        if($('input[id=proc'+i+']') =='on')
        {
            alert(";dd");
        }
        if(ads=='')
        {
            var ads=$('input[id=proc'+i+']:checked').val();
        }
        else
        {
            var ads=ads+';'+$('input[id=proc'+i+']:checked').val();
        }
        alert(ads);
        //}
    ...
    //var ads=$('input[id=proc1]:checked').val();
    //$aas=$("input[name='proc[]']:checked").length();
    //alert(ads);
...
<div id="middle">
<form action="/sreseption" method="post" name="add_klients" id="jform">
...
<tr class="vstr1 otkz" style="display: none; background: #A8F3AE;"> 
  ...
  <!--<tr style="background: #EDF3A8;" class="otkz">
    ...
    <option value="0" <?php echo set_select('perezvon', '0', TRUE); ?>>Нет</option>
    <option value="1" <?php echo set_select('perezvon', '1'); ?>>Да</option>
    </select>
    </td>
  </tr>
