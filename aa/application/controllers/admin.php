                    ...
                    if ($this->csvimport->get_array($file_path)) {
                        ...
                        foreach ($csv_array as $row) {
                            $insert_data = array(
                                'fio'=>$row['Fio'],
                                'phone'=>$row['Phone'],
                                'date_vstr'=>$row['date_vstr'],
                                'time_vstr'=>$row['time_vstr'],
                                'user'=>$row['user'],
                                'otkaz'=>$row['otkaz'],
                            );
...
//вроде общий отчет по кол-ву процедур
   ...
   public function otchet_proc()
{
    //проверяем авторизирован ли пользователь
	   //$this->auth_lib->check_user();
        ...
        $this->load->view('header_view');
		$this->load->view('left_view');
		$this->load->view('admin/otchet_proc');
		$this->load->view('footer_view');
}
...
echo $this->email->print_debugger();
  ...
  function safe_info_msg()
 {
   ...
   $this->auth_lib->check_user();
    ...
    if(isset($_POST['safe_info_msg']))
    {
        $query=$this->user_model->safe_info_msg($_POST['text_msg'],$_POST['color_bg_msg']);
        if($query==TRUE)
        {$status=0;echo $status;}
        else
        {$status=1;echo $status;}
