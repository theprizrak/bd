...
class Sreseption extends CI_Controller {
	...
	public function index()
	{
	   ...
	   $this->auth_lib->check_user();
        ...
        if(isset($_GET['filter']) &&$_GET['filter']!=''){$filter=$_GET['filter'];}else{$filter='none';}
        $data = array(
        'klient_vstr' =>$this->stelemarket_model->get_klients_vstr_resep1($config['per_page'],$this->uri->segment(3),$filter),
        'info' => '0'
        );
